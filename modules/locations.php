...
class LocationsModule 
{
    ...
    public function save ( $data, $_mode )
    {
        ...
        /** */
            ...
            if ( $_mode == 'edit' ) {
                try {
                    $sql = 'SELECT '.$sql_cols.' FROM locations WHERE id = ?';
        ...
        /** Validations */
            ...
            if ( $skip == false ) {
                ...
                } elseif ( ($result = $validations->unique('locations.code', $inputs['code'], 'Code')) !== true ) {
                    $report_msgs['code'] = $result;
                }
            }
        ...
        /** Save */
            ...
            try {
                if ( $_mode == 'save' ) {
                    $sql = 'INSERT INTO locations ('.$sql_cols.') VALUES ('.$sql_values.')';
                    $save = $db_conn->prepare($sql)->execute($inputs);
                } else {
                    $sql = 'UPDATE locations SET '.$sql_upd_val.' WHERE id = :id';
                    $save = $db_conn->prepare($sql)->execute($inputs);
                }
            } catch(PDOException $e) {
                throw new Exception($e);
    ...
    public function delete ( $data )
    {
        ...
        $db_conn = $db->db_connect();
        try {
            $sql = 'SELECT COUNT(`id`) FROM `locations` WHERE `id` = ?';
